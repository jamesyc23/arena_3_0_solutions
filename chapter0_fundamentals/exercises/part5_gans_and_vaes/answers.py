...
def initialize_weights(model: nn.Module) -> None:
    ...
    for (name, module) in model.named_modules():
        ...
        elif any([isinstance(module, Module) for Module in [Conv2d, ConvTranspose2d, Linear]]):
...
class AutoencoderTrainer:
    ...
    def training_step(self, img: t.Tensor) -> t.Tensor:
        ...
        img_prime = self.model(img)
        loss = ((img - img_prime) ** 2).mean()
        loss.backward()
        self.optimizer.step()
        return loss
    ...
    def evaluate(self) -> None:
        ...
        '''
        holdout_prime = self.model(HOLDOUT_DATA)
        holdout_prime_np = holdout_prime.permute(0, 2, 3, 1).numpy()
        imgs = [wandb.Image(img) for img in holdout_prime_np]
        wandb.log({"images": imgs}, step=self.step)
...
class VAE(nn.Module):
    ...
    def sample_latent_vector(self, x: t.Tensor) -> Tuple[t.Tensor, t.Tensor, t.Tensor]:
        ...
        mu, logsigma = encoder_out[:, :self.latent_dim_size], encoder_out[:, self.latent_dim_size:]
...
class VAETrainer:
    ...
    def training_step(self, img: t.Tensor) -> t.Tensor:
        ...
        kl_div = ((t.exp(logsigma) ** 2 + mu ** 2 - 1) / 2 - logsigma).mean()
        loss = ((img - img_prime) ** 2).mean() + self.args.beta_kl * kl_div
        loss.backward()
        self.optimizer.step()
        return loss
    ...
    def evaluate(self) -> None:
        ...
        Evaluates model on holdout data, logs to weights & biases.
        '''
        holdout_prime, _, _ = self.model(HOLDOUT_DATA)
        holdout_prime_np = holdout_prime.permute(0, 2, 3, 1).numpy()
        imgs = [wandb.Image(img) for img in holdout_prime_np]
        wandb.log({"images": imgs}, step=self.step)
