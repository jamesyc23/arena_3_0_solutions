...
def unbroadcast(broadcasted: Arr, original: Arr) -> Arr:
    ...
    '''
    oshape = list(original.shape)
    bshape = list(broadcasted.shape)

    broadcast_dims = []
    dims_appended = 0

    while len(oshape) < len(bshape):
        oshape.insert(0, 1)
        dims_appended += 1

    for dim, (odim, bdim) in enumerate(zip(oshape, bshape)):
        if odim != bdim:
            broadcast_dims.append(dim)

    s = broadcasted.sum(
        axis=tuple([d for d in broadcast_dims if d >= dims_appended]),
        keepdims=True
    )
    s = s.sum(
        axis=tuple(range(dims_appended)),
        keepdims=False
    )
...
def multiply_forward(a: Union[Tensor, int], b: Union[Tensor, int]) -> Tensor:
    ...
    requires_grad = grad_tracking_enabled and any(
        (isinstance(arg, Tensor) and (arg.requires_grad or arg.recipe is not None)) for arg in (a, b)
    )
    out = np.multiply(arg_a, arg_b)
    out = tensor(out, requires_grad=requires_grad)
    if requires_grad:
        out.recipe = Recipe(np.multiply, (arg_a, arg_b), {}, {i: arg for i, arg in enumerate([a, b]) if isinstance(arg, Tensor)})
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        ...
        requires_grad = grad_tracking_enabled and is_differentiable and any(
            isinstance(arg, Tensor) and (arg.requires_grad or a.recipe is not None) for arg in args
        )
        out = numpy_func(*np_args, **kwargs)
        out = tensor(out, requires_grad=requires_grad)
        if requires_grad:
            recipe = Recipe(numpy_func, tuple(np_args), kwargs, {i: arg for i, arg in enumerate(args) if isinstance(arg, Tensor)})
            out.recipe = recipe
...
def topological_sort(node, get_children):
    ...
    seen = set()
    stack = set()
    def topological_sort_recursive_helper(node):
        if node in stack:
        ...
        for child in get_children(node):
            if child not in seen:
                top_order.extend(topological_sort_recursive_helper(child))
    ...
    return topological_sort_recursive_helper(node)
...
def sorted_computational_graph(tensor: Tensor) -> List[Tensor]:
    ...
    return topological_sort(
        tensor,
        get_children=lambda t: [] if t.recipe is None else list(t.recipe.parents.values())
    )[::-1]
...
def backprop(end_node: Tensor, end_grad: Optional[Tensor] = None) -> None:
    ...
    '''
    top_order = sorted_computational_graph(end_node)

    for t in top_order:
        t.grad = zeros(*t.array.shape)

    end_node.grad = (
        end_grad
        if end_grad is not None
        else Tensor(np.ones_like(end_node.array))
    )
    ...
    for t in top_order:
        ...
        for parent_pos, parent in t.recipe.parents.items():
            back_func = BACK_FUNCS.get_back_func(t.recipe.func, arg_position=parent_pos)
            assert t.grad is not None
            assert parent.grad is not None
            parent.grad.array += back_func(t.grad.array, t.array, *t.recipe.args, **t.recipe.kwargs)
...
def exp_back(grad_out: Arr, out: Arr, x: Arr) -> Arr:
    '''Backward function for f(x) = np.exp(x) elementwise.'''
    return grad_out * out
