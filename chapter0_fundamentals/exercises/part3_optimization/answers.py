...
def opt_fn_with_sgd(fn: Callable, xy: t.Tensor, lr=0.001, momentum=0.98, n_iters: int = 100):
    ...
    '''
    xy = xy.clone().detach().requires_grad_()
    path = []
...
class SGD:
    ...
    def step(self) -> None:
        ...
        '''
        for i, (param, g) in enumerate(zip(self.params, self.gs)):
            assert param.grad is not None, f"param.grad is None for param {i}"
            self.gs[i] = param.grad
            if self.lmda != 0:
                self.gs[i] += self.lmda * param
            self.gs[i] = self.mu * g + self.gs[i]
            param -= self.lr * self.gs[i]
...
class RMSprop:
    ...
    @t.inference_mode()
    def step(self) -> None:
        for i, (param, v, b) in enumerate(zip(self.params, self.vs, self.bs)):
            self.gs[i] = param.grad + self.lmda * param
            self.vs[i] = self.alpha * v + (1 - self.alpha) * self.gs[i] ** 2
            if self.mu > 0:
                self.bs[i] = self.mu * b + self.gs[i] / (t.sqrt(self.vs[i]) + self.eps)
                param -= self.lr * self.bs[i]
            else:
                param -= self.lr * self.gs[i] / (t.sqrt(self.vs[i]) + self.eps)
...
class Adam:
    ...
    def step(self) -> None:
        ...
        for i, (param, m, v) in enumerate(zip(self.params, self.ms, self.vs)):
            self.gs[i] = param.grad + self.lmda * param
            self.ms[i] = self.beta1 * m + (1 - self.beta1) * self.gs[i] 
            self.vs[i] = self.beta2 * v + (1 - self.beta2) * self.gs[i] ** 2
...
optimizer_list = [
    ...
    # (Adam, {"lr": 0.2, "betas": (0.9, 0.999), "weight_decay": 0.005}),
    (AdamW, {"lr": 0.001, "betas": (0.9, 0.999), "weight_decay": 0.005}),
]
