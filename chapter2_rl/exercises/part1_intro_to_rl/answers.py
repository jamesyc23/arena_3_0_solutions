...
class RewardAveraging(Agent):
    ...
    def reset(self, seed: int):
        ...
        self.Q = np.full((num_arms,), self.optimism).astype(np.float32)
...
class UCBActionSelection(Agent):
    ...
    def get_action(self):
        return (self.Q + self.c * np.sqrt(np.reciprocal(self.N + self.eps) * np.log(self.t))).argmax()
    ...
    def reset(self, seed: int):
        super().reset(seed)
        self.N = np.zeros((num_arms,))
        self.Q = np.zeros((num_arms,)).astype(np.float32)
        self.t = 1
...
def policy_eval_numerical(env: Environment, pi: Arr, gamma=0.99, eps=1e-8, max_iterations=10_000) -> Arr:
    ...
    for _ in range(max_iterations):
        ...
        value = (
            np.multiply(env.T[np.arange(env.num_states), pi, :], env.R[np.arange(env.num_states), pi, :]).sum(axis=1)
            + gamma * env.T[np.arange(env.num_states), pi, :].dot(value)
        )
        if (np.abs(old_value - value) < eps).all():
            break
    print(value.shape)
    return value
...
def policy_eval_exact(env: Environment, pi: Arr, gamma=0.99) -> Arr:
    ...
    '''
    r_i = np.multiply(env.T[np.arange(env.num_states), pi, :], env.R[np.arange(env.num_states), pi, :]).sum(axis=1)
    value = np.linalg.inv(np.eye(env.num_states) - gamma * env.T[np.arange(env.num_states), pi, :]).dot(r_i)
    return value
...
def policy_improvement(env: Environment, V: Arr, gamma=0.99) -> Arr:
    ...
    '''
    pi_better = (
        einops.einsum(env.T, env.R, "start_state action end_state, start_state action end_state -> start_state action")
        + gamma * einops.einsum(env.T, V, "start_state action end_state, end_state -> start_state action")
    ).argmax(axis=1)
    return pi_better
...
def find_optimal_policy(env: Environment, gamma=0.99, max_iterations=10_000):
    ...
    for _ in range(max_iterations):
        ...
        old_pi = pi.copy()
        pi = policy_improvement(env, value, gamma=gamma)
