...
rprint(table)
# %%
def reward_fn_char_count(generated_sample: Union[List[str], str], char: str = '.') -> Union[float, Float[Tensor, "batch"]]:
    '''
    ...
    if type(generated_sample) is str:
        return float(Counter(generated_sample)[char])
    else:
        return t.tensor([float(Counter(s)[char]) for s in generated_sample])
...
def compute_advantages(
    ...
    '''
    qs = t.cat((values[:, prefix_len:-1], rewards.unsqueeze(-1)), dim=1)
    vs = values[:, prefix_len-1:-1]
...
def calc_kl_penalty(
    ...
    '''
    p_logprobs = logits[:, prefix_len-1:-1, :].log_softmax(dim=-1)
    q_logprobs = ref_logits[:, prefix_len-1:-1, :].log_softmax(dim=-1)

    return kl_coef * (p_logprobs.exp() * (p_logprobs - q_logprobs)).sum(dim=-1).mean()
...
def calc_entropy_bonus(
    ...
    prefix_len:
        The length of the prefix to ignore when computing the KL divergence.
    '''
    logprobs = logits[:, prefix_len-1:-1, :].log_softmax(dim=-1)
    return ent_coef * -(logprobs.exp() * logprobs).sum(dim=-1).mean()
...
def get_logprobs(
    ...
    logprobs = t.gather(logits[:, prefix_len-1:-1].log_softmax(-1), 2, tokens[:, prefix_len:].unsqueeze(-1)).squeeze()
    return logprobs
...
class RLHFTrainer:
    ...
    def compute_rlhf_objective(self, mb: ReplayMinibatch):
        ...
        value_function_loss = calc_value_function_loss(values[:, self.prefix_len:], mb.returns, args.vf_coef)
    ...
    def rollout_phase(self) -> ReplayMemory:
        ...
        rewards = normalize_reward(self.args.reward_fn(samples)).to(device)
    ...
    def learning_phase(self, memory: ReplayMemory) -> None:
        ...
        for mb in mbs:
            ...
            rlhf_obj = self.compute_rlhf_objective(mb)
            rlhf_obj.backward()
            ...
            if self.args.use_wandb:
                ...
                mean_reward = self.args.reward_fn(self.model.base_model.to_string(mb.sample_ids)).mean()
                kl_penalty = calc_kl_penalty(logits, mb.ref_logits, self.args.kl_coef, self.prefix_len)
                entropy_bonus = calc_entropy_bonus(logits, self.args.ent_coef, self.prefix_len)
                wandb.log(
                    {
